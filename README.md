...
- 副本：宝库、梦境、怪异塔、换皮闯关
- 通知与定时：群广播、挂机提醒、护卫成员提醒、定时日常、定时资源/副本执行、活动开放提醒
- 周/月度专项定时：每周答题、月末月度任务提醒、月末钓鱼进度补齐、疯狂赛车禁发提醒、疯狂赛车智能发车、疯狂赛车夜间收车提醒、疯狂赛车收车
...
- `request_timeout` 为统一的 sidecar 调用超时（秒）；赛车收车提醒、挂机提醒、手动智能发车、定时日常都会在此基础上换算具体请求超时。
- `notify_mode` 为通知策略：
  - `group_broadcast_with_private_fallback`：优先发到已绑定通知群，失败后自动降级私聊
  - `group_broadcast`：仅发到已绑定通知群
...
- 疯狂赛车收车提醒与收车默认为每日 `23:55`，也支持自定义每日时间。
...
当前手动智能发车为独立 sidecar 逻辑；`/xyzw 赛车 智能发车` 与 `定时 赛车 智能发车` 都复用这套逻辑。
...
- 每辆车最多刷新 5 次
- 单车刷新或发车失败会自动重试 1 次
- 超级跑车生效时，直发判断会跳过“当前持有刷新券数量”分支；当无券且无免费刷新时，可额外使用最多 5 次超级跑车刷新
